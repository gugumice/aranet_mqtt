...
run_time = timedelta(seconds=1*60)
...
def write_to_file(sd: dict, csvfile_name: str):
    fields = ['SensorId','SensorName','Group','rssi','Batt%','time']
    with open(csvfile_name, 'w') as csvfile:
        ...
        for k,v in sd.items():
            ...
            if 'measurements' in v:
                dt_obj = datetime.fromtimestamp(v['measurements']['time'])
                row=row+[v['measurements']['rssi'],v['measurements']['battery'],dt_obj.strftime('%Y-%m-%d %H:%M:%S')]
            print(row)
            writer.writerow(row)
